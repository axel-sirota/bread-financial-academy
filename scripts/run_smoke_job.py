"""Upload the Week 20 smoke script to Databricks and run it as a one-off job.

The smoke logic (scripts/smoke/week20_env_smoke_job.py) must run ON the course
cluster so it exercises real dbutils, spark, and the cluster's network egress.
This runner uploads it, submits a one-off job run, polls to completion, and
prints the run output.

Usage:
    python3 scripts/run_smoke_job.py --cluster-id 0512-181411-pzxqam5e
"""
...
SMOKE_SRC = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "smoke", "week20_env_smoke_job.py"
)
WORKSPACE_PATH = "/Shared/bread_academy/week20_env_smoke"
...
def main():
    ...
    # 1. Upload the smoke script as a workspace notebook (SOURCE/PYTHON).
    ...
    # 4. Pull the run output. For a multi-task run the notebook output and
    #    dbutils.notebook.exit() value live on the TASK run, not the parent,
    #    so fetch get-output for each task run id.
    detail = requests.get(
        f"{host}/api/2.1/jobs/runs/get",
        headers=h, params={"run_id": run_id}, timeout=60,
    ).json()
